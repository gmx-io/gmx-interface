...
function Leaderboard() {
  ...
  const { gtRankStats, isLoading: gtRankStatsLoading } = useGtRankStats();
  ...
  const { userRankInfo } = useGtRankStatForUser(publicKey?.toBase58());
  const [showVipTiers, setShowVipTiers] = useState(false);
  const isLoading = gtRankStatsLoading;
  ...
  const sortedUserRanks = useMemo(() => {
    if (!gtRankStats?.userRanks || gtRankStats.userRanks.length === 0) {
      return [];
    }
    if (!currentUserBase58) {
      return [...gtRankStats.userRanks];
    }

    const currentUser = gtRankStats.userRanks.find(
      (user) => user.owner.toBase58() === currentUserBase58
    );
    ...
    const otherUsers = gtRankStats.userRanks.filter(
      (user) => user.owner.toBase58() !== currentUserBase58
    );

    return [currentUser, ...otherUsers];
  }, [gtRankStats?.userRanks, currentUserBase58]);

  const visibleUserRanks = useMemo(() => {
    return sortedUserRanks.slice(0, MAX_DISPLAY_ITEMS);
  }, [sortedUserRanks]);

  const currentPageItems = useMemo(() => {
    const startIndex = (page - 1) * LEADERBOARD_LIST_PER_PAGE;
    const endIndex = startIndex + LEADERBOARD_LIST_PER_PAGE;
    return visibleUserRanks.slice(startIndex, endIndex);
  }, [visibleUserRanks, page]);
  ...
  const currentUserInList = useMemo(() => {
    ...
    return gtRankStats?.userRanks?.find(
      (user) => user.owner.toBase58() === currentUserBase58
    );
  }, [gtRankStats?.userRanks, currentUserBase58]);

  const pageCount = Math.ceil(
    visibleUserRanks.length / LEADERBOARD_LIST_PER_PAGE
  );

  useEffect(() => {
    if (!visibleUserRanks.length) {
      return undefined;
    }

    const prefetchPages = [page + 1, page - 1].filter(
      (prefetchPage) => prefetchPage >= 1 && prefetchPage <= pageCount
    );

    if (!prefetchPages.length) {
      return undefined;
    }

    const prefetchAccountInfo = () => {
      prefetchPages.forEach((prefetchPage) => {
        const startIndex = (prefetchPage - 1) * LEADERBOARD_LIST_PER_PAGE;
        const endIndex = startIndex + LEADERBOARD_LIST_PER_PAGE;

        visibleUserRanks.slice(startIndex, endIndex).forEach((user) => {
    ...
    if (window.requestIdleCallback) {
      const idleCallbackId = window.requestIdleCallback(prefetchAccountInfo);

      return () => window.cancelIdleCallback(idleCallbackId);
    }

    const timeoutId = window.setTimeout(prefetchAccountInfo, 0);

    return () => window.clearTimeout(timeoutId);
  }, [page, pageCount, visibleUserRanks]);
  ...
  return (
    <>
      <div className="gt-leaderboard bg-[#181818]">
        ...
        {gtRankStats.userRanks.length > 0 && (
          <div
            style={{

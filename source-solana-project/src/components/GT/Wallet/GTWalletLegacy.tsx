...
function GTWallet({ linkBuyBack }: { linkBuyBack: () => void }) {
  const { store } = useStoreAccount(GMX_SOLANA_STORE_ADDRESS);
  const { gtHistory, isLoading: isGtHistoryLoading } = useGtHistoryData();
  ...
  const filteredHistory = useMemo(() => {
    const list = gtHistory || [];
    if (selectedActionKeys.length === 0) {
      return list;
    }
    return list.filter((item: any) => {
      const key = item?.action as 'mint' | 'sell' | 'referral_rewards' | 'stake';
      return selectedActionKeys.includes(key);
    });
  }, [gtHistory, selectedActionKeys]);
  const sortedHistory = useMemo(() => {
    const mode = sortFieldList[sortField];
    if (!filteredHistory || !mode || mode === 'default') return filteredHistory || [];
    ...
    return arr;
  }, [filteredHistory, sortField, sortFieldList]);
  const pageCount = Math.ceil((sortedHistory?.length || 0) / LEADERBOARD_LIST_PER_PAGE);
  const currentPageItems = useMemo(() => {
    const startIndex = (page - 1) * LEADERBOARD_LIST_PER_PAGE;
    const endIndex = startIndex + LEADERBOARD_LIST_PER_PAGE;
    const slice = sortedHistory?.slice(startIndex, endIndex) || [];
  ...
  useEffect(() => {
    if (gtHistory?.length) {
      const referralTotalBN = gtHistory.reduce((acc, item) => {
        return item?.action === 'referral_rewards' ? acc.add(item?.amountBN) : acc;
      }, BN_ZERO);
      const mintTotalBN = gtHistory.reduce((acc, item) => {
        return item?.action === 'mint' ? acc.add(item?.amountBN) : acc;
      }, BN_ZERO);
      const stakeTotalBN = gtHistory.reduce((acc, item) => {
        return item?.action === 'stake' ? acc.add(item?.amountBN) : acc;
      }, BN_ZERO);
      setStakeAccount(formatAmount(stakeTotalBN, gtDecimals, 4, true, true));
      setReferralAccount(formatAmount(referralTotalBN, gtDecimals, 4, true, true));
      setTradeAccount(formatAmount(mintTotalBN, gtDecimals, 4, true, true));
    }
  }, [gtHistory]);

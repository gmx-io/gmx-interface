...
const GT_EARNED_TOTALS_KEY = 'data_store/gt_earned_totals';
...
export const useGtEarnedTotals = (enabled = true) => {
  ...
  const shouldFetch = enabled && isGmw331Enabled;
  const tradingBN = useAppStore(selectGtUserDetailsTotalMinted);
  const { owner } = useAnchor();

  const { data, isLoading } = useSWR(
    shouldFetch && owner ? [GT_EARNED_TOTALS_KEY, owner.toBase58()] : null,
    async ([, ownerAddress]: [string, string]) => {
      try {
        ...
        return { referralBN: BN_ZERO, stakeBN: BN_ZERO };
      }
    },

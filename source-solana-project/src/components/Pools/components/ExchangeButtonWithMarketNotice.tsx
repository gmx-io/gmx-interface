...
import { getRwaMarketCategory } from '@/components/Pools/utils/getRwaMarketCategory';
...
function ExchangeButton({ params }: any) {
  ...
  const isGlv = isGlvBugSingle || isGlvBuyPair || isGlvSell;
  const isUs = isRestrictedArea();
  const { icon: warningIcon } = getNoticeConfig('info');
  const marketCategory = useMemo(
    () => getRwaMarketCategory(poolInfo?.glvToken, poolInfo?.indexToken),
    [poolInfo?.glvToken, poolInfo?.indexToken]
  );
  const marketNotOpenNoticeTitle = useMemo(() => {
    if (marketCategory === 'stock') {
      return t`Buying and selling the corresponding GLV/GM is unavailable during off-hours (outside 9:30–16:00 ET on trading days) and market holidays.`;
    }
    if (marketCategory === 'forex') {
      return t`Buying and selling the corresponding GLV/GM is unavailable during off-hours (outside Sun 17:00 - Fri 17:00 ET) and market holidays.`;
    }
    if (marketCategory === 'commodity') {
      return (
        <div>
          <p>{t`Buying and selling the corresponding GLV/GM is unavailable during non-trading periods:`}</p>
          <p style={{ margin: '0.5rem 0' }}>• {t`Daily break: 17:00 – 18:00 ET`}</p>
          <p style={{ marginBottom: '0.5rem' }}>• {t`Outside Sun 18:00 – Fri 17:00 ET`}</p>
          <p>• {t`Market holidays`}</p>
        </div>
      );
    }
    return null;
  ...
  const [isMarketNotOpenNoticeDismissed, setIsMarketNotOpenNoticeDismissed] =
    useState(false);
  const [signature, setSignature] = useState<string>('');
  const [successPayDetails, setSuccessPayDetails] = useState<string[]>([]);

  useEffect(() => {
    setIsMarketNotOpenNoticeDismissed(false);
  }, [poolInfo?.glvToken, poolInfo?.marketToken, poolInfo?.indexToken]);
  ...
  const handleMarketNotOpenNoticeClose = () => {
    setIsMarketNotOpenNoticeDismissed(true);
  };
  const buttonContent = (
    <>
      {isMarketNotOpen &&
        showMarketNotOpenNotice &&
        !isMarketNotOpenNoticeDismissed &&
        marketCategory &&
        marketNotOpenNoticeTitle && (
          <NoticeCard
            type="info"
            cardType="notice"
            className={`mb-[1rem] mt-[-0.2rem] w-full px-[1.2rem] ${isGlv ? '!px-0' : ''}`}
            title={marketNotOpenNoticeTitle}
            icon={warningIcon}
            closeIconClass="w-[1.6rem] h-[1.6rem]"
            onClose={handleMarketNotOpenNoticeClose}
          />
        )}

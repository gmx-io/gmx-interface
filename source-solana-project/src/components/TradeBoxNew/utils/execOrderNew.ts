...
import type { ExecOrderResult } from '@/components/TradeBoxNew/utils/execOrderToast';

export type { ExecOrderResult } from '@/components/TradeBoxNew/utils/execOrderToast';
export {
    isExecOrderSuccess,
    getIncreaseTpslPendingMessage,
    getExecOrderResultMessage,
} from '@/components/TradeBoxNew/utils/execOrderToast';
...
function deserializeTransaction(item: Uint8Array | Buffer) {
    try {
        ...
        try {
            ...
            throw new Error(
                `Transaction deserialization failed: ${(versionedError as Error).message}`
            );
        }
    }
}
...
function pushUniqueKind(list: CreateOrderKind[], kind: CreateOrderKind) {
    if (!list.includes(kind)) {
        list.push(kind);
    }
}

function markLegSuccess(
    succeeded: CreateOrderKind[],
    failed: CreateOrderKind[],
    kind: CreateOrderKind
) {
    if (!failed.includes(kind)) {
        pushUniqueKind(succeeded, kind);
    }
}

function markLegFailed(
    succeeded: CreateOrderKind[],
    failed: CreateOrderKind[],
    kind: CreateOrderKind
) {
    const idx = succeeded.indexOf(kind);
    if (idx >= 0) {
        succeeded.splice(idx, 1);
    }
    pushUniqueKind(failed, kind);
}
...
export const useExecOrder = async (
...
): Promise<ExecOrderResult> => {
    const sendRawTransactionOpts = getSendRawTransactionOpts();
    const connection = storeProgram.provider.connection;
    const intended = orderParams.map((item) => item.kind);
    const succeeded: CreateOrderKind[] = [];
    const failed: CreateOrderKind[] = [];
    const signatures: string[] = [];
    const errors: unknown[] = [];
    const tsx: Array<Uint8Array | Buffer> = [];
    const kindsPerTx: CreateOrderKind[] = [];
    ...
    for (const item of orderParams) {
        ...
        try {
            ...
            const serialized = transactions.serialize().flat();
            for (const tx of serialized) {
                tsx.push(tx);
                kindsPerTx.push(kind);
            }
        } catch (error) {
            console.log('create_orders error', error);
            errors.push(error);
            markLegFailed(succeeded, failed, kind);
        }
    }
    ...
    if (tsx.length === 0) {
        return { succeeded, failed, signatures, intended, errors };
    }

    try {
        const versionedTxns = tsx.map((item) => deserializeTransaction(item));
        ...
        const confirmAndRecord = async (
            ...
            try {
                const serializedTx = serializeSignedTransaction(signedTransaction);
                ...
                if (status.status === 'processed') {
                    markLegSuccess(succeeded, failed, kind);
                    return true;
                }
                errors.push(new Error(status.message || `Transaction confirmation failed: ${status.status}`));
                markLegFailed(succeeded, failed, kind);
                return false;
            } catch (sendError) {
        ...
        if (signedTx.length > 1) {
            const firstOk = await confirmAndRecord(signedTx[0], kindsPerTx[0]);
            if (!firstOk) {
                for (let index = 1; index < kindsPerTx.length; index++) {
                    markLegFailed(succeeded, failed, kindsPerTx[index]);
                }
            } else {
                for (let index = 1; index < signedTx.length; index++) {
                    const ok = await confirmAndRecord(
                        signedTx[index],
                        kindsPerTx[index]
                    );
                    if (!ok) {
                        for (let rest = index + 1; rest < kindsPerTx.length; rest++) {
                            markLegFailed(succeeded, failed, kindsPerTx[rest]);
                        }
                        break;
                    }
                }
            ...
            for (let index = 0; index < signedTx.length; index++) {
                await confirmAndRecord(signedTx[index], kindsPerTx[index]);
            }
        }

        console.log('result', { succeeded, failed, signatures });
        return { succeeded, failed, signatures, intended, errors };
    } catch (error) {
        console.error('Transaction processing error:', error);
        for (const kind of kindsPerTx) {
            markLegFailed(succeeded, failed, kind);
        }
        throw error;
    }
...
export const useExecUpdateOrder = async (
    ...
    const tsx = [...transactions.serialize().flat()];
    console.log('tsx', tsx);
    try {
        const versionedTxns = tsx.map((item) => deserializeTransaction(item));
        const signedTx = await signAllTransactions(versionedTxns);
        ...
        return await Promise.all(
            signedTx.map(async (signedTransaction, index) => {
                try {
                    const serializedTx = serializeSignedTransaction(signedTransaction);
                    const sig = await connection.sendRawTransaction(
                        serializedTx,
                    ...
                    if (status.status !== 'processed') {
                        throw new Error(
                            status.message ||
                                `Transaction ${index + 1} confirmation failed: ${status.status}`
                        );
                    }
                    return sig;
                } catch (sendError) {

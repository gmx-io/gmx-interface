...
function useScrollFade(
  getSnapChildren: (scrollable: HTMLDivElement) => HTMLElement[]
) {
  ...
  const scrollTo = useCallback(
    (dir: 1 | -1) => {
      ...
      const targetScrollLeft = currentScrollLeft + proposedScrollLeft;
...
function ScrollFadeControls({
  ...
  const toColor =
    gradientColor === 'slate-800' ? 'to-slate-800' : 'to-slate-900';

  const [absoluteRef, { height: absoluteHeight }] =
  ...
  return (
    <div
      className="pointer-events-none absolute flex h-full w-full flex-row justify-between "
      ref={absoluteRef}
    >
      <div
        className={cx(
          'max-w-50 group z-[120] h-full cursor-pointer transition-opacity items-center',
      ...
      >
        {scrollLeft > 0 && (
          <div
            className={cx(
              'sticky py-12',
              isContainerLarge ? 'top-1/2' : 'bottom-0 top-0'
            )}
          >
            <BiChevronLeft
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        ref={tableScrollFade.scrollableRef}
      >
        {children}
      </div>
    </div>
...
export function BodyScrollFadeContainer({
  children,
  className,
  ...
  return (
    <div className="relative">
      ...
      <div
        className={cx('scrollbar-hide overflow-x-auto', className)}
        ref={scrollFade.scrollableRef}
      >
        {children}
      </div>
    </div>

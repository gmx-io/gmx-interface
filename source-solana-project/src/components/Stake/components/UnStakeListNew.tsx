...
} from '../Hooks/useStakeHistoryData';
...
function UnStakeList({
  ...
  // Sort all already-loaded rows (e.g. page 1+2), then slice the active page.
  ...
  const currentPageUnStakes = useMemo(() => {
    const startIndex = (page - 1) * STAKE_HISTORY_PAGE_SIZE;
    return sortedData.slice(startIndex, startIndex + STAKE_HISTORY_PAGE_SIZE);
  }, [sortedData, page]);

  const loadedPageCount = Math.max(
    1,
    Math.ceil(sortedData.length / STAKE_HISTORY_PAGE_SIZE)
  );
  const pageCount = hasMore
    ? Math.max(page + 1, loadedPageCount)
    : loadedPageCount;
              ...
              <TableTh>
                <div
                  className={`unstake-table-sort-wrapper sortable ${
                    gmSortField === 'amount' &&
                    gmSortFieldList['amount'] !== 'default'
                      ? 'sort-select'
                      : ''
                  }`}
...
const UnStakeItemWrapper = memo(
  ({ index, position }: { index: number; position: StakeHistoryItem }) => {
    const isMobile = useMedia('(max-width: 768px)');

    const [longSymbol = '', shortSymbol = ''] =
      position?.symbolPair?.split('-') || [];
      ...
      >
        ...
        <TableTd className="!w-[25rem]">
          <div className="token">
            <div className="token-symbol">
              ...
              {position?.poolType === 'GM' && (
                <div className="ls-img">
                  <img
                    src={getIconUrlPath(longSymbol, 24)}
                    alt="long"
                    width={18}
                  />
                  <img
                    src={getIconUrlPath(shortSymbol, 24)}
                    alt="short"
                    width={18}
                  />
                </div>
            ...
            <div className="info">
              <p
                className={`text-p mb-[0.2rem] ${isMobile ? '!text-[1.3rem]' : ''}`}
              >
                {position?.poolType === 'GM'
        ...
        <TableTd className={isMobile ? 'w-[10rem]' : 'w-[25%]'}>
          <p className="text-p">
            {formatAmount(position?.amount, position?.decimals, 2)}
          </p>
        </TableTd>

...
export const defaultChartProps = {
  ...
  custom_formatters: {
    ...
    dateFormatter: {
      ...
      parse: (date) =>
        lightFormat(parse(date, 'dd MMM yyyy', new Date()), 'YYYY-MM-DD'),
    },
    ...
    priceFormatterFactory: (symbolInfo) => {
      ...
      return {
        format: (price) => {
          ...
          const precision = symbolInfo?.name.includes('EUR') || symbolInfo?.name.includes('GBP') || symbolInfo?.name.includes('USDMXN') ? 5 : getPriceDecimals(bn);

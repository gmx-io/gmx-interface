...
export function getFundingFactorPerPeriod(marketInfo: MarketInfo, isLong: boolean, periodInSeconds: number) {
  ...
  const fundingForPayingSide = (fundingFactorPerSecond * largerInterestUsd) / payingInterestUsd;
  const fundingForReceivingSide = (fundingForPayingSide * payingInterestUsd) / receivingInterestUsd;

...
import { TableFilterBase } from '@/components/Common/Table/TableFilterBase/TableFilterBase';
import { defined, definedOrThrow } from '@/utils/lib/assertions';
...
import { FlatItems } from './flat';
import { Groups } from './groups';
import { useFilteredFlatItems, useFilteredGroups } from './hooks';
...
export function TableOptionsFilter<T>({
  ...
  const isGrouped =
    options.length > 0 && 'groupName' in options[0] && 'items' in options[0];
  ...
  const handleSetValue = useCallback((value: string) => setSearch(value), []);
  ...
  const handleGroupToggle = useCallback(
    (group: FilteredGroup<T>) => {
      ...
      if (group.isEverythingFilteredSelected) {
        onChange(
          ensuredValue.filter(
            (pair) => !group.items.some((item) => isEqual(pair, item.data))
          )
        );
      } else {

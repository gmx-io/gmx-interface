...
export const selectOrderErrorsByOrderKeyMap = createAppStoreSelector(
  [
    selectOrdersInfo,
    selectPositionsInfo,
    selectMarketsInfo,
  ...
  (
    ...
    const result: Record<string, OrderErrors> = {};

    if (!marketsInfo || !ordersInfo || !wrappedNativeToken) {
      return result;
    }

    Object.entries(ordersInfo).forEach(([orderKey, orderInfo]) => {

...
import type { FindSwapPath } from "../../../../../sdk/src/utils/trade/types";
...
function makeMockSwapPathStats(usdOut: bigint, amountOut: bigint) {
  return {
    swapPath: ["0xmarket1"],
    swapSteps: [],
...
function makeMockFindSwapPath(usdOut: bigint, amountOut: bigint): FindSwapPath {
  return (_usdIn) => makeMockSwapPathStats(usdOut, amountOut);
}

function makeNoPathFindSwapPath(): FindSwapPath {
...
describe("isThirdTokenDecreaseSwap", () => {
  it("returns true when all three tokens are different", () => {
    expect(isThirdTokenDecreaseSwap(wethToken, usdcToken, arbToken)).toBe(true);
  });

  it("returns false when receiveToken equals pnlToken", () => {
    expect(isThirdTokenDecreaseSwap(wethToken, usdcToken, ethToken)).toBe(false);
  });

  it("returns false when receiveToken equals collateralToken", () => {
    expect(isThirdTokenDecreaseSwap(wethToken, usdcToken, usdcToken)).toBe(false);
  });

  it("returns false when pnlToken equals collateralToken", () => {
    expect(isThirdTokenDecreaseSwap(wethToken, ethToken, arbToken)).toBe(false);
  });

  it("returns false when all tokens are the same", () => {
    expect(isThirdTokenDecreaseSwap(ethToken, ethToken, ethToken)).toBe(false);
  });
});
...
describe("getDecreaseSwapType", () => {
  it("returns NoSwap when pnl equals collateral", () => {
    expect(getDecreaseSwapType(wethToken, ethToken, usdcToken)).toBe(DecreasePositionSwapType.NoSwap);
  });

  it("returns SwapCollateralTokenToPnlToken when pnl equals receive", () => {
    expect(getDecreaseSwapType(wethToken, usdcToken, ethToken)).toBe(
      DecreasePositionSwapType.SwapCollateralTokenToPnlToken
    );
  });

  it("returns SwapPnlTokenToCollateralToken otherwise", () => {
    expect(getDecreaseSwapType(wethToken, usdcToken, arbToken)).toBe(

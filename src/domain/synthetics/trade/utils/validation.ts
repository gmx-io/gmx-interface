...
import { isDepositDisabledMarket, isDepositDisabledGlv } from "config/static/markets";
...
export function getGmSwapError(p: {
  ...
  if (isDeposit && glvInfo && isDepositDisabledGlv(chainId, glvInfo.glvTokenAddress)) {
    return { buttonErrorMessage: t`Buying GLV is disabled for this vault` };
  }

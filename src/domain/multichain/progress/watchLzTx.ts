...
export async function watchLzTxRpc({
  ...
  const destinationChainId = ENDPOINT_ID_TO_CHAIN_ID[oftSentEvent.args.dstEid as keyof typeof ENDPOINT_ID_TO_CHAIN_ID];
...
function getLzStatusFromApiResponse(operation: LzApiOperation): LzStatus {
  ...
  const destinationChainId = operation.pathway?.dstEid
    ? ENDPOINT_ID_TO_CHAIN_ID[operation.pathway.dstEid as keyof typeof ENDPOINT_ID_TO_CHAIN_ID]
    : undefined;

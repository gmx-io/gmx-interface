...
export const selectPositionsInfoDataSortedByMarket = createSelector((q) => {
  ...
  const sortedPositions = positions.sort((a, b) => {
    const aMarketIdx = marketsSortMap[a.market.indexTokenAddress as keyof typeof marketsSortMap];
    const bMarketIdx = marketsSortMap[b.market.indexTokenAddress as keyof typeof marketsSortMap];

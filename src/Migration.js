...
function MigrationModal(props) {
  const { isVisible, setIsVisible, isPendingApproval, setIsPendingApproval, value, setValue, index, balances, active, account, library } = props
  const token = tokens[index]
  const [isMigrating, setIsMigrating] = useState(false)
  const [isApproving, setIsApproving] = useState(false)

  const { data: tokenAllowance, mutate: updateTokenAllowance } = useSWR([active, CHAIN_ID, token.address, "allowance", account, gmxMigratorAddress], {
  ...
  const isPrimaryEnabled = () => {
    ...
    if (needApproval && isPendingApproval) { return false }
    return true
  }
  ...
  const getPrimaryText = () => {
    ...
    if (needApproval && isPendingApproval) { return "Waiting for Approval" }
    if (needApproval) { return `Approve ${token.name}` }
      ...
      <Modal isVisible={isVisible} setIsVisible={setIsVisible} label={`Migrate ${token.name}`}>
        <div className="Exchange-swap-section">
          <div className="Exchange-swap-section-top">
            ...
            <div className="muted align-right clickable" onClick={() => setValue(formatAmountFree(maxAmount, 18, 8))}>Max: {formatAmount(maxAmount, 18, 4, true)}</div>
          </div>
        ...
        <div className="MigrationModal-info-box">
          <div className="App-info-row">
            ...
            <div className="align-right">
              {baseAmount && `${formatAmount(baseAmount, 18, 4, true)} GMX ($${formatAmount(baseAmountUsd, 18 + decimals, 2, true)})`}
              {!baseAmount && "-"}
            </div>
          </div>
          {token.bonus > 0 &&
            <div className="App-info-row">
              ...
              <div className="align-right">
                {bonusAmount && `${formatAmount(bonusAmount, 18, 4, true)} GMX ($${formatAmount(bonusAmountUsd, 18 + decimals, 2, true)})`}
                {!bonusAmount && "-"}
              </div>
            </div>
          }
          {token.bonus > 0 &&
            <div className="App-info-row">
              ...
              <div className="align-right">
                {totalAmount && `${formatAmount(totalAmount, 18, 4, true)} GMX ($${formatAmount(totalAmountUsd, 18 + decimals, 2, true)})`}
                {!totalAmount && "-"}
              </div>
            </div>
          }
...
export default function Migration() {
  ...
  const { data: iouBalances, mutate: updateIouBalances } = useSWR(["Migration:iouBalances", CHAIN_ID, readerAddress, "getTokenBalancesWithSupplies", account || AddressZero], {
    fetcher: fetcher(library, Reader, [iouTokenAddresses]),
  })

  const { data: balances, mutate: updateBalances } = useSWR(["Migration:balances", CHAIN_ID, readerAddress, "getTokenBalancesWithSupplies", account || AddressZero], {
    fetcher: fetcher(library, Reader, [tokenAddresses]),
  })

  const { data: migratedAmounts, mutate: updateMigratedAmounts } = useSWR(["Migration:migratedAmounts", CHAIN_ID, gmxMigratorAddress, "getTokenAmounts"], {
  ...
  }, [active, library, updateBalances, updateIouBalances,
      updateMigratedAmounts])
  ...
  return(
    <div className="Migration Page">
      ...
      <div className="Migration-note">
        Your wallet: {formatAmount(gmxBalance, 18, 4, true)} GMX
      </div>
      <div className="Migration-note">
        Please read the&nbsp;
        <a href="https://gambitprotocol.medium.com/gambit-gmx-migration-now-live-2ba999d208dd" target="_blank" rel="noopener noreferrer">
          Medium post
        </a> before migrating.
      </div>
            ...
            <div className={cx("border", "App-card", { primary: index === 0 })} key={index}>
              ...
              <div className="Stake-card-bottom App-card-content">
                <div className="Stake-info App-card-row">
                  ...
                  <div>
                    {formatArrayAmount(balances, index * 2, 18, 4, true)}
                  </div>
                </div>
                <div className="Stake-info App-card-row">
                  <div className="label">Migration Price</div>
                  <div>
                    ${formatAmount(price, decimals, 2, true)}
                  </div>
                </div>
                <div className="Stake-info App-card-row">
                ...
                <div className="Stake-info App-card-row">
                  ...
                  {!hasCap && <div>
                    {formatArrayAmount(migratedAmounts, index, 18, 0, true)}
                  </div>}
                  {hasCap && <div>
                ...
                <div className="App-card-options">
                  ...
                  {active && <button className="App-button-option App-card-option" onClick={() => showMigrationModal(index)}>Migrate</button>}
                </div>
              </div>
            </div>
          )

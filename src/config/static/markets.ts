...
export const MARKETS = Object.keys(MARKETS_UI_CONFIGS).reduce(
  (acc, network) => {
    const key = network as `${ContractsChainId}`;
    return {
      ...acc,
      [network]: Object.keys(MARKETS_UI_CONFIGS[key]).reduce((acc, address) => {
        return {
          ...acc,
          [address]: {
            ...SDK_MARKETS[key][address],
            ...MARKETS_UI_CONFIGS[key][address],
          },
        };
      }, {}),
    };

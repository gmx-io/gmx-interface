...
function getIconUrlPath(symbol: string | undefined) {
  if (!symbol) return;
...
function TokenIcon({ className, symbol, displaySize, badge, badgeClassName, chainIdBadge }: Props) {
  ...
  if (badge) {
      ...
      sub = (
        ...
        >
          <img
            ...
            src={importImage(getIconUrlPath(badge[0]) as string)}
            alt={badge[0]}
            width={20}
            height={20}
          />
          <img
            ...
            src={importImage(getIconUrlPath(badge[1]) as string)}
            alt={badge[0]}
            width={20}
            height={20}
          />
    ...
    sub = (
      <img
        src={(CHAIN_ID_TO_NETWORK_ICON as any)[chainIdBadge]}
        width={size}
        height={size}

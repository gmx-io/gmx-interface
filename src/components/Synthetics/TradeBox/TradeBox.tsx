...
export function TradeBox({ isMobile }: { isMobile: boolean }) {
    ...
    function tradeBoxInteractionStartedEffect() {
      if (fromTokenInputValue.length > 0) {
        ...
        if (isIncrease && increaseAmounts) {
          ...
          priceImpactDeltaUsd = swapAmounts.swapSettings.swapPathStats?.totalSwapPriceImpactDeltaUsd ?? 0n;
  ...
  if (showDebugValues) {
    const swapPathStats = swapAmounts?.swapSettings.swapPathStats || increaseAmounts?.swapSettings.swapPathStats;

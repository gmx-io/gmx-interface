  ...
  return (
    <div>
      {isMobile && (
        <>
          {positions.length === 0 && (
            <div className="App-card text-slate-500">{isLoading ? t`Loading...` : t`No open positions`}</div>
          )}
          ...
          <tbody>
            {positions.length === 0 && (
              <TableTr hoverable={false} bordered={false}>
                <TableTd colSpan={15}>
                  <div className="text-slate-500">{isLoading ? t`Loading...` : t`No open positions`}</div>
                </TableTd>
              </TableTr>
            )}
            {!isLoading &&

...
export function CompositionDiagram({ data, label }: Props) {
  const chartData = useMemo((): CompositionChartItem[] => {
    ...
    return data.map((item) => {
      ...
      const color =
        (item.type === "market"
          ? TOKEN_COLOR_MAP[item.market.indexToken.symbol as keyof typeof TOKEN_COLOR_MAP]
          : TOKEN_COLOR_MAP[item.token.symbol as keyof typeof TOKEN_COLOR_MAP]) ?? TOKEN_COLOR_MAP.default;
      return {

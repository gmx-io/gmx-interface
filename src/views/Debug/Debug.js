...
export default function Debug() {
  ...
  const { data: uniPoolSlot0, mutate: updateUniPoolSlot0 } = useSWR([`Debug:uniPoolSlot0:${active}`, chainId, poolAddress, "slot0"], {
    fetcher: fetcher(library, UniPool),
  })

  const { data: uniPoolTickSpacing, mutate: updateUniPoolTickSpacing } = useSWR([`Debug:uniPoolTickSpacing:${active}`, chainId, poolAddress, "tickSpacing"], {
    fetcher: fetcher(library, UniPool),
  })
  ...
  }, [library, active, updateUniPoolSlot0,
      updateUniPoolTickSpacing])
